...
public class EMObjectGroup<T> extends IDObject{
    ...
    private static class SpecificClassGroup<S>{
        ...
        public void setTClass(Class<S> tClass) {
            this.tClass = tClass;
        }
    ...
    /**
     * 获取指定代理class的代理对象
     * @param tClass 代理的class
     * @return 代理对象
     */
    public EMProxyHolder<? super T> getProxyHolder(Class<? super T> tClass){
        return getGroupByClass(tClass).getProxyHolder();
    }
     ...
     * @return 代理对象列表
     */
    public List<? extends EMObjectInfo<? super T,?>> getMockInfo(Class<? super T> tClass){
    ...
    private <S> void proxyHolderUpdate(EMProxyHolder<S> holder){
        getGroupByClass(holder.gettClass()).setProxyHolder(holder);
    }

    private <S> void mockInfoUpdate(EMObjectInfo<S,?> emObjectInfo){
        List<EMObjectInfo<S, ?>> mockObjects = getGroupByClass(emObjectInfo.getDefinition().getTClass()).getMockObjects();
